...
-- Script to extract Triggers & Trigger Functions belongs to BILLADMIN role to execute on Postgresq1
...
BEGIN
  ...
  FOR rec IN (
    ...
    FROM all_triggers
    WHERE owner = 'BILLADMIN'  ORDER BY TRIGGER_NAME-- Replace with your schema name
  ) LOOP
    ...
    v_create_function_sql := 'CREATE OR REPLACE FUNCTION ' ||v_trigger_name || '_function() RETURNS TRIGGER AS $$' || CHR(10) ||
                             'BEGIN' || CHR(10) ||
