...
-- Script to extract Primary Keys belongs to BILLADMIN role to execute on Postgresq1
...
FROM (
    ...
    WHERE 
        c.constraint_type = 'P' -- Filter for primary keys
        AND c.owner = 'BILLADMIN'      -- Replace 'OPER' with your schema name
...
SELECT 
    'ALTER TABLE ' || owner || '.' || table_name || 
    ' ADD CONSTRAINT ' || constraint_name || ' UNIQUE (' || LISTAGG(column_name, ', ') 
    WITHIN GROUP (ORDER BY column_position) || ');' AS ddl_statement
FROM (
    SELECT 
        c.table_name AS table_name,
