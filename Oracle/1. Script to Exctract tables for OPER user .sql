...
-- Script to extract tables belongs to OPER role and all datatypes will be converted to map Postgresq1
    ...
    FOR tbl_rec IN tbl_cursor LOOP
        ...
        FOR col_rec IN col_cursor(tbl_rec.table_name) LOOP
			...
			ELSIF col_rec.data_type IN (
                ...
                IF col_rec.data_default IS NOT NULL THEN
					v_column_definition := v_column_definition || 'BYTEA(' || col_rec.data_length || ')' ||' DEFAULT  ' || col_rec.data_default ||'';
				ELSE
					v_column_definition := v_column_definition || 'BYTEA(' || col_rec.data_length || ')' ;
				END IF;	
